"""
benchmark.py — External benchmark dataset runner for the SurvPFN project.
...
CLI
---
    uv run python benchmark.py --datasets SUPPORT2 METABRIC GBSG --models all
    uv run python benchmark.py --datasets METABRIC --models cox rsf gbsa deepsurv mtlr
    uv run python benchmark.py --datasets GBSG --tune --trials 20
...
from survpfn.benchmark_datasets import BENCHMARK_DATASETS
from survpfn.metrics import evaluate_survival_model
...
def run_cox(df_train, df_test, tune, n_trials):
    from survpfn.cox_models import run_multivariate_cox
...
def run_rsf(df_train, df_test, tune, n_trials):
    from survpfn.tree_models import train_rsf
    from survpfn.metrics import evaluate_survival_model
...
def run_gbsa(df_train, df_test, tune, n_trials):
    from survpfn.tree_models import train_gbsa
...
def run_mtlr(df_train, df_test, tune, n_trials):
    from survpfn.discrete_surv import train_mtlr
...
def run_pchazard(df_train, df_test, tune, n_trials):
    from survpfn.discrete_surv import train_pchazard
...
def run_deephit_single(df_train, df_test, tune, n_trials):
    from survpfn.discrete_surv import train_deephit_single
